...
autoCompleteInputTag.addEventListener("keyup", (event) => {
  ...
  resultContainerTag.innerHTML = ""; // the empty string if nothing to happened
  if(searchText.length === 0) { // remove the array if the input is empty.
    return;
  }
  
  filterProducts = products.filter((product) => { // using filter() method to the array.
    return product.title.toLowerCase().includes(searchText);  // includes() = check the userinput when user typed
  });
...
const navigateAndSelectProduct = (key) => {
  if (key === "ArrowDown"){ 
    if(indexToSelect === filterProducts.length -1) { // the last for products
      indexToSelect = -1; 
      deselectProduct();
      return;
    }
    indexToSelect += 1; // plus 1 if ArrowDown
    const productItemContainerToSelect = selectProduct (indexToSelect);

    if(indexToSelect > 0) { 
      const productIdToSelect = document.getElementsByClassName("selected")[0];
      productIdToSelect.style.backgroundColor = "white";
      productIdToSelect.firstChild.style.color = "black";
      productIdToSelect.classList.remove("selected")  // delete class name of "selected" if click to dobule tap
    }

    productItemContainerToSelect.classList.add("selected"); //want to know where div reach
...
const deselectProduct = () => {
  ...
  productIdToSelect.style.backgroundColor = "white";
  productIdToSelect.firstChild.style.color = "black";
  productIdToSelect.classList.remove("selected");
}